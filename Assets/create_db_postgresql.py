    ...
    def __init__(self):
        if DB_RESET:
            ...
            for producer in LIST_PRODUCER:
                ...
                con.commit()
            # sets parameters to 0 for unsupported producer
...
class Employees:
    ...
    def __init__(self, number_of_employees):
        if DB_RESET:
            Employees.__create_table_employees()
    ...
    @classmethod
    def __create_table_employees(cls):
...
class EmployeesInProject:
    ...
    def __init__(self):
        if DB_RESET:
            EmployeesInProject.__create_table_for_employees_in_use()
        ...
        for project in list_of_projects_id:
            ...
            number_of_employees_in_projects = random.randint(1, int(NUMBER_OF_EMPLOYEES / 3))

            for y in range(number_of_employees_in_projects):
                cur.execute('''INSERT INTO {}(
                               uuid_employee,
                               project_id)
                               VALUES (
    ...
    @classmethod
    def __create_table_for_employees_in_use(cls):
...
class RandomInformation:
    ...
    def __give_last_name(cls, gender):
        # returns random last_name argument needs to be M - male F - female
...
class CreateUsers:
    # creates users and updates theirs profile
    ...
    def __save_users(cls):
        # saves users information's(username, password)
...
def populate_db():
    conn, cur = connect_to_db(HOST, USER, PASSWORD, PORT, NAME_DB)
...
def create_json_file():
    # creates json file with data to manually update into django information's(posts)
    informations = []
...
def create_informations_db(number_of_inf):
    # adds information's(posts) to DB through PostgreSQL
    list_of_users = list_of_active_users()
    conn, cur = connect_to_db(HOST, USER, PASSWORD, PORT, NAME_DB)
...
def remove_users_and_informations():
    # remove users and information's
...
if __name__ == "__main__":
    option = input(f"What you want to do?"
                   ...
                   f"\n2-   Produce dummy date ({NUMBER_OF_INFORMATION_posts} information's(posts))"
                   f"\n3-   Add {NUMBER_OF_USERS_TO_ADD} users"
                   f"\n4-   Create {NUMBER_OF_USERS_TO_ADD} new users and {NUMBER_OF_INFORMATION_posts} "
                   f"information's(posts)"
                   f"\n5-   Remove all users and information's-posts "
                   f"(and restart auto_id for users and information's(posts))\n"
                   f""
    ...
    if int(option) == 1:
        ...
        choice = input('\nHow you wanna add information\'s(posts) to the DB?'
                       '\n1-     Manual through JSON file'
                       '\n2-     Automatically through PostgeSQL'
                       '(check if the table name for information = {})\n'.format(TABLE_NAME_FOR_INFORMATION))
        if int(choice) == 1:
            create_json_file()
        elif int(choice) == 2:
        ...
        decision = input("Are you sure you want to remove users and information's(posts)?"
        ...
        print('Now users will be created'
              '\n Don\'t do anything after you write "next"')
        ...
        DB_RESET = False
        CreateUsers(NUMBER_OF_USERS_TO_ADD)
        print('Now information\'s(posts) will be created')
